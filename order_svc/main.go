...
import (
	...
	"gokit-ddd-demo/order_svc/infras/inmem"
)
...
func main() {
	...
	orderRepo.InitMockData([]*order.Order{
		&order.Order{ID: 1, UserID: 1, Product: "product1"},
		&order.Order{ID: 2, UserID: 1, Product: "product2"},
		&order.Order{ID: 3, UserID: 2, Product: "product1"},
	})

	var (
		grpcServer  = apigrpc.NewGRPCServer(orderSvc, logger)
		httpHandler = apihttp.NewHTTPHandler(orderSvc, logger)
	)
